"""Backend-independent primitives shared by Dreaming and Memory Hygiene.
...
This inward-facing module also owns the consolidation result value object
and deterministic centroid math.  It deliberately imports no storage backend
or optional extension implementation, so infrastructure and extensions can
share the same definitions without depending on each other.
...
class ConsolidationResult:
    ...
    Attributes:
        ...
        promoted_count: Number of thoughts that passed gates and threshold.
        promoted_ids: Thought IDs that were promoted.
        skipped_gate_count: Thoughts that failed gate checks.
        scores: Mapping of thought ID to computed weighted score.
        ...
        promotion_capped: Whether promotion stopped at the P1 fraction cap.
        p1_fraction_after: Fraction of the corpus at P1 after this run.
        orphans_retired: Number of orphaned reflections retired this run.
        active_signal_weights: Effective per-signal weights after inactive
            signal redistribution.
        flat_signals: Sorted names of configured signals that were inactive.
...
def compute_centroid(member_vectors: list[list[float]]) -> list[float]:
    ...
    unit length. When the mean is the zero vector, it is returned unchanged
...
def default_signal_active(
    ...
    *,
    ...
    redistributed onto the active signals by the consuming scorer.
